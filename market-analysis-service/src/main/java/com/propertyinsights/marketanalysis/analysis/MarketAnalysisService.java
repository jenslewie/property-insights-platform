...
public class MarketAnalysisService {
  ...
  public DistributionResponse distribution(SegmentFilter filter, DistributionDimension dimension) {
    List<PropertyRecord> allRows = dataset.all();
  ...
  private MarketSummary calculateSummary(List<PropertyRecord> rows, int totalCount) {
    if (rows.isEmpty()) {
      return new MarketSummary(totalCount, 0, new MarketSummary.PriceStats(null, null, null, null));
    }
    ...
    return new MarketSummary(
        totalCount, rows.size(), new MarketSummary.PriceStats(mean, median, minimum, maximum));
  }
    ...
    for (PropertyRecord row : allRows) {
      ...
      BucketAccumulator bucket =
          buckets.stream()
              ...
              .findFirst()
              .orElseThrow(
                  () -> new IllegalStateException("Property did not match a distribution bucket."));

      bucket.add(row.price());
    }

    return new DistributionResponse(
        dimension, matchedCount, buckets.stream().map(BucketAccumulator::toBucket).toList());
  }
  ...
  private List<BucketTemplate> bucketTemplates(
    ...
    return switch (dimension) {
      ...
      case SQUARE_FOOTAGE ->
          rangeTemplates(new BigDecimal("1200"), new BigDecimal("1600"), new BigDecimal("2000"));
      case YEAR_BUILT ->
          rangeTemplates(
              new BigDecimal("1980"),
              new BigDecimal("1990"),
              ...
              new BigDecimal("2010"));
      case LOT_SIZE ->
          rangeTemplates(new BigDecimal("6000"), new BigDecimal("8000"), new BigDecimal("10000"));
  ...
  private List<BucketTemplate> rangeTemplates(BigDecimal... cutPoints) {
    ...
    for (BigDecimal upper : cutPoints) {
      String key = lower == null ? "lt_" + token(upper) : token(lower) + "_to_" + token(upper);
      String label = lower == null ? "<" + number(upper) : number(lower) + "–<" + number(upper);
    ...
    return values.stream()
        .map(
            value -> {
              ...
              return new BucketTemplate("exact_" + token(value), canonical, null, null, value);
            })
        .toList();
  }
  ...
  private static final class BucketAccumulator {
    ...
    private DistributionBucket toBucket() {
      BigDecimal averagePrice =
          count == 0
              ? null
              : priceSum.divide(BigDecimal.valueOf(count), MONEY_SCALE, RoundingMode.HALF_UP);
