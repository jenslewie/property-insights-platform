...
public final class PdfReportWriter {
    ...
    private static final String TITLE = "Property Market Analysis - CSV sample";
    ...
    public void write(ReportData report, OutputStream output) throws IOException {
        try (PDDocument document = new PDDocument()) {
            try (PageWriter pages = new PageWriter(document)) {
                ...
                pages.writeLine(
                        "Historical sample statistics from the loaded CSV dataset.",
                        pages.bodyFont(),
                        10);
                pages.writeLine(
                        "Predictions are not included in this report.", pages.bodyFont(), 10);
                pages.writeLine(
                        "Generated at (UTC): " + report.generatedAt(), pages.bodyFont(), 10);
    ...
    private static final class PageWriter implements AutoCloseable {
        ...
        private void writeLine(String text, PDFont font, float fontSize) throws IOException {
            ...
            do {
                ...
                writePhysicalLine(text.substring(start, end), font, fontSize);
                start = end;
            } while (start < text.length());
        }

        private void writePhysicalLine(String text, PDFont font, float fontSize)
                throws IOException {
            if (stream == null) {
                startPage(false);
            }
