...
class PriceImpactApiTest {
  ...
  void setUp() {
    org.mockito.Mockito.when(dataset.all())
        .thenReturn(
            List.of(
                property(1, 1000, 2, "8"), property(2, 1200, 3, "9"), property(3, 1400, 3, "10")));
    doAnswer(
            invocation ->
                ((List<HousingFeatures>) invocation.getArgument(0))
                    .stream()
                        .map(features -> features.schoolRating().multiply(new BigDecimal("100")))
                        .toList())
    ...
    mockMvc
        .perform(
            post("/api/v1/market/price-impact")
                ...
                .content(request("{\"min_bedrooms\":3}", "{\"school_rating_delta\":1}")))
    ...
    verify(predictionClient)
        .predict(
            argThat(
                features ->
                    features.size() == 4
                        && features.get(0).schoolRating().compareTo(new BigDecimal("9")) == 0
                        && features.get(1).schoolRating().compareTo(new BigDecimal("10")) == 0
                        && features.get(2).schoolRating().compareTo(new BigDecimal("10")) == 0
                        && features.get(3).schoolRating().compareTo(new BigDecimal("11")) == 0));
  }
  ...
  void filtersBeforeScenarioAdjustments() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/market/price-impact")
                .contentType(MediaType.APPLICATION_JSON)
                .content(request("{\"min_school_rating\":9}", "{\"school_rating_delta\":1}")))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.property_count").value(2));
  ...
  void returnsNullPercentageForZeroBaselineAndPreservesNegativeImpact() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.impact.mean.percentage_change").value(org.hamcrest.Matchers.nullValue()));
  ...
  void rejectsEmptySegmentNoOpAndOutOfBoundsScenarioWithoutModelCall() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/market/price-impact")
                .contentType(MediaType.APPLICATION_JSON)
                .content(request("{\"min_bedrooms\":9}", "{\"school_rating_delta\":1}")))
