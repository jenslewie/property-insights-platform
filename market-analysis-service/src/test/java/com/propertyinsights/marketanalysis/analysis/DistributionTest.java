...
class DistributionTest {
  ...
  void cutPointsBelongToUpperBucket() {
    ...
    DistributionResponse result =
        service(rows).distribution(SegmentFilter.unrestricted(), DistributionDimension.PRICE);
    ...
    assertThat(result.buckets())
        ...
        .containsExactly(
            "lt_200000", "200000_to_250000", "250000_to_300000", "300000_to_350000", "gte_350000");
  }
  ...
  void zeroCountBucketsRemainAndHaveNullAveragePrice() {
    ...
    DistributionResponse result =
        service(rows)
            .distribution(new SegmentFilterParser().parse(params), DistributionDimension.PRICE);
  ...
  void bedroomAndBathroomBucketsComeFromTheFullDataset() {
    ...
    DistributionResponse bedrooms = service.distribution(filter, DistributionDimension.BEDROOMS);
    DistributionResponse bathrooms = service.distribution(filter, DistributionDimension.BATHROOMS);

    assertThat(bedrooms.buckets())
        .extracting(DistributionBucket::label)
        .containsExactly("2", "3", "4");
    assertThat(bedrooms.buckets()).extracting(DistributionBucket::count).containsExactly(0, 0, 1);

    assertThat(bathrooms.buckets())
        .extracting(DistributionBucket::label)
        .containsExactly("1", "2", "2.5");
    assertThat(bathrooms.buckets()).extracting(DistributionBucket::count).containsExactly(0, 0, 1);
  }
  ...
  private static Stream<Arguments> rangeDimensionsAndCutPoints() {
    return Stream.of(
        ...
        Arguments.of(
            ...
            List.of(new BigDecimal("1200"), new BigDecimal("1600"), new BigDecimal("2000")),
            List.of(1, 2, 2, 1)),
        Arguments.of(
        ...
        Arguments.of(
            ...
            List.of(new BigDecimal("6000"), new BigDecimal("8000"), new BigDecimal("10000")),
            List.of(1, 2, 2, 1)),
        Arguments.of(
  ...
  private static BigDecimal stepFor(DistributionDimension dimension) {
    return switch (dimension) {
      ...
      case BEDROOMS, BATHROOMS ->
          throw new IllegalArgumentException("Discrete dimensions do not have range cut points.");
    };
  }
